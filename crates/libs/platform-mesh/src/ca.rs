...
impl MeshCa {
    ...
    pub async fn init(pool: &PgPool, config: &MeshConfig) -> Result<Self, MeshError> {
        let master_key = resolve_master_key(config)?;
        let root_ttl_days = config.root_ttl_days;
        let cert_ttl_secs = config.cert_ttl_secs;
...
fn resolve_master_key(config: &MeshConfig) -> Result<[u8; 32], MeshError> {
    let key_hex = config
        .master_key_hex
        .as_deref()
        .ok_or_else(|| MeshError::CaInit("PLATFORM_MASTER_KEY required for mesh CA".into()))?;
    platform_secrets::parse_master_key(key_hex)
...
async fn load_root_key(
    ...
    let plaintext = platform_secrets::decrypt(&row.encrypted_value, master_key, None)
...
async fn store_root_key(
    ...
    let encrypted = platform_secrets::encrypt(root_key_pem.as_bytes(), master_key)
