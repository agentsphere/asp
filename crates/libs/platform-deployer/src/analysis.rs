...
use crate::state::{DeployerState, ReconcilerServices};
use crate::types::{AnalysisVerdict, MetricGate};
...
pub async fn run<Svc: ReconcilerServices>(
    ...
    state.task_heartbeat.register("analysis_loop", 30);
    ...
    loop {
        tokio::select! {
            ...
            _ = interval.tick() => {
                state.task_heartbeat.heartbeat("analysis_loop");
                ...
                async {
                    if let Err(e) = tick(&state).await {
                        state.task_heartbeat.report_error("analysis_loop", &e.to_string());
...
async fn tick<Svc: ReconcilerServices>(state: &DeployerState<Svc>) -> anyhow::Result<()> {
...
async fn analyze_release<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release_id: Uuid,
    project_id: Uuid,
    step_index: i32,
...
async fn ensure_analysis_record<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release_id: Uuid,
    step_index: i32,
    rollout_config: &serde_json::Value,
...
async fn check_rollback_triggers<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    project_id: Uuid,
    triggers: &[MetricGate],
    ...
    for trigger in triggers {
        ...
        if let Some(val) = value
            && state.services.check_condition(
                &trigger.condition,
...
async fn evaluate_progress_gates<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    project_id: Uuid,
    rollout_config: &serde_json::Value,
    ...
    for gate in &progress_gates {
        ...
        let passed = match value {
            Some(val) => !state.services.check_condition(
...
async fn count_project_requests<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    project_id: Uuid,
) -> u64 {
...
async fn evaluate_gate_metric<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    project_id: Uuid,
    gate: &MetricGate,
