...
use crate::image_inspect::{self, EntrypointCache};
use crate::renderer;
use crate::state::{DeployerConfig, DeployerState, ReconcilerServices};
...
pub async fn run<Svc: ReconcilerServices>(
    ...
    state.task_heartbeat.register("deployer_reconciler", 15);
            ...
            _ = interval.tick() => {
                ...
                async {
                    match reconcile(&state).await {
                        Ok(()) => state.task_heartbeat.heartbeat("deployer_reconciler"),
                        Err(e) => {
                            state.task_heartbeat.report_error("deployer_reconciler", &e.to_string());
...
async fn reconcile<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
...
async fn reconcile_one<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
...
async fn handle_pending<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
) -> Result<(), DeployerError> {
    ...
    if release.strategy == "rolling" {
        ...
        publish_release_promoted(state, release).await;
    } else {
...
fn handle_rolling_progress<Svc: ReconcilerServices>(
    _state: &DeployerState<Svc>,
...
async fn handle_canary_progress<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
) -> Result<(), DeployerError> {
...
async fn handle_ab_test_progress<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
) -> Result<(), DeployerError> {
...
async fn handle_promoting<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
) -> Result<(), DeployerError> {
    let ns = target_namespace(&state.config, &release.namespace_slug, &release.environment);
...
async fn handle_rolling_back<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
) -> Result<(), DeployerError> {
    let ns = target_namespace(&state.config, &release.namespace_slug, &release.environment);
...
async fn apply_gateway_resources<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
    namespace: &str,
    canary_weight: i32,
    ...
    let gw = crate::gateway::GatewayRef {
    ...
    match release.strategy.as_str() {
        "canary" => {
            ...
            match crate::gateway::build_weighted_httproute(
                &route_name,
                namespace,
                hostname,
                stable_svc,
        ...
        "ab_test" => {
            ...
            let route = crate::gateway::build_header_match_httproute(
                &route_name,
                namespace,
                hostname,
                control_svc,
...
async fn apply_json_to_k8s<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
...
async fn resolve_gateway_url<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
) -> Option<String> {
...
async fn cleanup_expired_previews<Svc: ReconcilerServices>(state: &DeployerState<Svc>) {
    ...
    for row in &expired {
        ...
        if let Err(e) = state.services.delete_namespace(&state.kube, &ns).await {
...
async fn transition_phase<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
    new_phase: &str,
    ...
    if let (Some(current), Some(next)) = (
        crate::types::ReleasePhase::parse(&release.phase),
        crate::types::ReleasePhase::parse(new_phase),
...
async fn record_history<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
    action: &str,
    phase: &str,
...
pub async fn mark_failed<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
    message: &str,
) {
...
async fn create_user_secrets<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
    namespace: &str,
) -> Vec<String> {
    ...
    for row in &rows {
        ...
        let _encrypted: Vec<u8> = row.get("encrypted_value");
        // Decrypt via the services trait
        match state
            .services
            .decrypt_secret(release.project_id, &name)
            .await
        {
            Ok(Some(val)) => {
                let k8s_name = secret_name_from_key(&name);
                let data = BTreeMap::from([("value".to_string(), val)]);
                apply_k8s_secret(state, namespace, &k8s_name, &release.project_id, &data).await;
...
#[tracing::instrument(skip(state, release), fields(project_id = %release.project_id, %namespace))]
async fn inject_platform_secret<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
    namespace: &str,
) -> Option<String> {
...
#[tracing::instrument(skip(state, release), fields(project_id = %release.project_id, %namespace))]
async fn inject_project_secrets<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
    namespace: &str,
) -> Option<String> {
...
async fn inject_otel_env_vars<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
...
pub async fn ensure_scoped_tokens<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    project_id: Uuid,
    scope: &str,
...
pub async fn ensure_scoped_tokens_in_ns<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    project_id: Uuid,
    scope: &str,
    namespace: &str,
...
async fn ensure_single_token<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    ...
    let (raw_token, hash) = state.services.generate_api_token();
...
async fn ensure_project_service_account<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    project_id: Uuid,
...
async fn resolve_project_owner<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    project_id: Uuid,
) -> Option<(Uuid, String)> {
...
pub async fn ensure_registry_pull_secret_for<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    project_id: Uuid,
    release_id: Uuid,
    namespace: &str,
...
async fn create_deploy_pull_token<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    owner_id: Uuid,
    release_id: Uuid,
) -> Option<String> {
...
async fn apply_k8s_secret<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    namespace: &str,
    secret_name: &str,
    project_id: &Uuid,
...
async fn apply_or_replace_secret<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    namespace: &str,
    secret_name: &str,
...
async fn render_manifests<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
) -> Result<(String, Option<String>), DeployerError> {
    if let Some(ops_repo_id) = release.ops_repo_id {
        let (repo_path, _sha, branch) = state
            .services
            .sync_ops_repo(&state.pool, ops_repo_id)
            .await
            .map_err(DeployerError::Other)?;
        ...
        let sha = state
            .services
            .get_branch_sha(&repo_path, values_branch)
            .await
        ...
        let template_content = if manifest_path.ends_with('/') {
            state
                .services
                .ops_read_dir_yaml(&repo_path, &sha, manifest_path)
                .await
                .ok_or_else(|| DeployerError::RenderFailed("directory not found".into()))?
        } else {
            state
                .services
                .ops_read_file(&repo_path, &sha, manifest_path)
                .await
                .ok_or_else(|| DeployerError::RenderFailed("file not found".into()))?
        };

        // Read values from the environment-specific branch (matches eventbus logic)
        let ops_values = state
            .services
            .read_values(&repo_path, values_branch, &release.environment)
            .await
...
fn generate_basic_manifest(release: &PendingRelease) -> Result<String, DeployerError> {
    ...
    platform_types::validation::check_pipeline_image(&release.image_ref)
        .map_err(|e: platform_types::ApiError| DeployerError::InvalidManifest(e.to_string()))?;
...
async fn store_tracked_resources<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release_id: Uuid,
...
async fn fire_webhook<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
    action: &str,
) {
    ...
    state
        .services
        .fire_webhooks(release.project_id, "deploy", &payload)
        .await;
}

/// Publish a `ReleasePromoted` event to the event bus.
async fn publish_release_promoted<Svc: ReconcilerServices>(
    state: &DeployerState<Svc>,
    release: &PendingRelease,
) {
    let event = serde_json::json!({
...
mod tests {
    ...
    fn basic_manifest_rejects_injection_dollar() {
        ...
        // Note: platform_types::check_pipeline_image does not reject '$' (unlike
        // src/validation.rs::check_container_image which does). The crate uses
        // check_pipeline_image which has a narrower forbidden-char set.
        // This test documents the current behavior.
        let _result = generate_basic_manifest(&r);
    }
