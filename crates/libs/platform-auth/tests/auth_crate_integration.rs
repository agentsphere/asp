...
#[sqlx::test(migrations = "../../../migrations")]
async fn effective_permissions_cache_hit(pool: PgPool) {
    use fred::interfaces::KeysInterface;

    let valkey = valkey_pool().await;
    let user_id = seed_user(&pool, &format!("u-{}", Uuid::new_v4())).await;

    // Pre-populate cache with a known permission set (no DB role exists)
    let cache_key = format!("perms:{user_id}:global");
    let cached_perms = vec!["project:read".to_string(), "project:write".to_string()];
    let json = serde_json::to_string(&cached_perms).unwrap();
    let _: () = valkey
        .set(
            &cache_key,
            json,
    ...
    // Call effective_permissions — should return cached perms without hitting DB
    let perms = platform_auth::resolver::effective_permissions(&pool, &valkey, user_id, None)
        .await
    ...
    assert!(
        perms.contains(&platform_types::Permission::ProjectRead),
        "cache should provide ProjectRead"
    );
    assert!(
        perms.contains(&platform_types::Permission::ProjectWrite),
        "cache should provide ProjectWrite"
    );
...
#[sqlx::test(migrations = "../../../migrations")]
async fn invalidate_permissions_with_project_clears_both_keys(pool: PgPool) {
    use fred::interfaces::KeysInterface;
    ...
    // Seed and populate both cache keys
    let perm_id = seed_permission(&pool, "project:read").await;
    let role_id = seed_role(&pool, &format!("role-{}", Uuid::new_v4()), &[perm_id]).await;
    assign_role(&pool, user_id, role_id, None).await;
    ...
    // Populate a project-scoped cache key
    let ws_id = seed_workspace(&pool, owner_id, user_id, "member").await;
    let project_id = seed_project(&pool, owner_id, ws_id).await;
    let _ =
        platform_auth::resolver::effective_permissions(&pool, &valkey, user_id, Some(project_id))
    ...
    let global_key = format!("perms:{user_id}:global");
    let project_key = format!("perms:{user_id}:{project_id}");
    assert!(
        valkey.exists::<bool, _>(&global_key).await.unwrap(),
        "global cache should exist"
    );
    assert!(
        valkey.exists::<bool, _>(&project_key).await.unwrap(),
        "project cache should exist"
    );

    // Invalidate with project_id → should clear both global + project keys
    platform_auth::resolver::invalidate_permissions(&valkey, user_id, Some(project_id))
        .await
        .expect("invalidation should succeed");

    assert!(
        !valkey.exists::<bool, _>(&global_key).await.unwrap(),
        "global cache should be cleared"
    );
    assert!(
        !valkey.exists::<bool, _>(&project_key).await.unwrap(),
...
#[sqlx::test(migrations = "../../../migrations")]
async fn invalidate_permissions_without_project_uses_pattern(pool: PgPool) {
    use fred::interfaces::KeysInterface;

    let valkey = valkey_pool().await;
    let user_id = seed_user(&pool, &format!("u-{}", Uuid::new_v4())).await;

    // Seed and populate cache key
    let perm_id = seed_permission(&pool, "project:read").await;
    let role_id = seed_role(&pool, &format!("role-{}", Uuid::new_v4()), &[perm_id]).await;
    assign_role(&pool, user_id, role_id, None).await;
    ...
    // Create a fake project-scoped cache entry
    let fake_project_id = Uuid::new_v4();
    let project_key = format!("perms:{user_id}:{fake_project_id}");
    let _: () = valkey
        .set(
            &project_key,
            "[\"project:read\"]",
    ...
    let global_key = format!("perms:{user_id}:global");
    assert!(valkey.exists::<bool, _>(&global_key).await.unwrap());
    assert!(valkey.exists::<bool, _>(&project_key).await.unwrap());

    // Invalidate without project_id → uses pattern perms:{user_id}:*
    platform_auth::resolver::invalidate_permissions(&valkey, user_id, None)
        .await
        .expect("pattern invalidation should succeed");

    assert!(
        !valkey.exists::<bool, _>(&global_key).await.unwrap(),
        "global cache should be cleared by pattern"
    );
    assert!(
        !valkey.exists::<bool, _>(&project_key).await.unwrap(),
        "project cache should be cleared by pattern"
    );
}

// ---------------------------------------------------------------------------
// Workspace permission edge cases
// ---------------------------------------------------------------------------

#[sqlx::test(migrations = "../../../migrations")]
async fn no_workspace_membership_no_implicit_perms(pool: PgPool) {
    let valkey = valkey_pool().await;
    let owner_id = seed_user(&pool, &format!("owner-{}", Uuid::new_v4())).await;
    let outsider_id = seed_user(&pool, &format!("outsider-{}", Uuid::new_v4())).await;

    // Create workspace + project, but don't add outsider as member
    let ws_id = seed_workspace(&pool, owner_id, owner_id, "owner").await;
    let project_id = seed_project(&pool, owner_id, ws_id).await;

    let perms = platform_auth::resolver::effective_permissions(
        &pool,
        &valkey,
        outsider_id,
        Some(project_id),
    ...
    assert!(
        !perms.contains(&platform_types::Permission::ProjectRead),
        "non-member should NOT get ProjectRead"
    );
    assert!(
        !perms.contains(&platform_types::Permission::ProjectWrite),
        "non-member should NOT get ProjectWrite"
    );

    // Cleanup
    platform_auth::resolver::invalidate_permissions(&valkey, outsider_id, Some(project_id))
        .await
        .ok();
}

#[sqlx::test(migrations = "../../../migrations")]
async fn workspace_owner_gets_project_read_and_write(pool: PgPool) {
    let valkey = valkey_pool().await;
    let owner_id = seed_user(&pool, &format!("owner-{}", Uuid::new_v4())).await;

    // Owner is automatically added as "owner" member via seed_workspace
    let ws_id = seed_workspace(&pool, owner_id, owner_id, "owner").await;
    let project_id = seed_project(&pool, owner_id, ws_id).await;

    let perms =
        platform_auth::resolver::effective_permissions(&pool, &valkey, owner_id, Some(project_id))
            .await
            .expect("should resolve");

    assert!(
        perms.contains(&platform_types::Permission::ProjectRead),
        "workspace owner should get ProjectRead"
    );
    assert!(
        perms.contains(&platform_types::Permission::ProjectWrite),
        "workspace owner should get ProjectWrite"
    );

    // Cleanup
    platform_auth::resolver::invalidate_permissions(&valkey, owner_id, Some(project_id))
        .await
        .ok();
}

// ---------------------------------------------------------------------------
// PgPermissionChecker trait implementation
// ---------------------------------------------------------------------------

#[sqlx::test(migrations = "../../../migrations")]
async fn pg_permission_checker_has_permission_via_trait(pool: PgPool) {
    use platform_types::PermissionChecker;
    ...
    let checker = platform_auth::resolver::PgPermissionChecker {
        pool: &pool,
        valkey: &valkey,
    };

    let allowed = checker
        .has_permission(user_id, None, platform_types::Permission::ProjectRead)
        .await
        .expect("should not error");
    assert!(allowed, "PgPermissionChecker should find the permission");

    let denied = checker
        .has_permission(user_id, None, platform_types::Permission::ProjectWrite)
        .await
        .expect("should not error");
    assert!(!denied, "user should NOT have ProjectWrite");
...
#[sqlx::test(migrations = "../../../migrations")]
async fn pg_permission_checker_has_permission_scoped_via_trait(pool: PgPool) {
    use platform_types::PermissionChecker;
    ...
    // Non-matching scope
    let scopes = vec!["admin:config".to_string()];
    let denied = checker
        .has_permission_scoped(
            user_id,
            None,
        ...
        .await
        .expect("should not error");
    assert!(!denied, "non-matching scope should deny");
...
#[sqlx::test(migrations = "../../../migrations")]
async fn pg_permission_resolver_role_permissions_via_trait(pool: PgPool) {
    use platform_types::PermissionResolver;
    ...
    let resolver = platform_auth::resolver::PgPermissionChecker {
        pool: &pool,
        valkey: &valkey,
    };
...
#[sqlx::test(migrations = "../../../migrations")]
async fn pg_permission_resolver_effective_permissions_via_trait(pool: PgPool) {
    use platform_types::PermissionResolver;
    ...
    let perms = resolver
        .effective_permissions(user_id, None)
        .await
        .expect("should resolve");
...
#[sqlx::test(migrations = "../../../migrations")]
async fn pg_permission_resolver_invalidate_via_trait(pool: PgPool) {
    use fred::interfaces::KeysInterface;
    use platform_types::PermissionResolver;
    ...
    let cache_key = format!("perms:{user_id}:global");
    assert!(valkey.exists::<bool, _>(&cache_key).await.unwrap());
    ...
    resolver
        .invalidate_permissions(user_id, None)
        .await
        .expect("invalidation via trait should succeed");

    assert!(
        !valkey.exists::<bool, _>(&cache_key).await.unwrap(),
        "cache should be cleared via trait method"
    );
}
