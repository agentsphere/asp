...
use platform_secrets::user_keys;
...
pub async fn create_session(
    ...
    let session_ns = platform_k8s::namespace::session_namespace_name(
    ...
    platform_k8s::namespace::ensure_session_namespace(
        &state.kube,
        &session_ns,
    ...
    let registry_pull_secret = if let Some(reg_url) = node_registry_url {
        match platform_registry::pull_secret::create_pull_secret(
            &state.pool,
            &state.kube,
            reg_url,
            user_id,
    ...
    let registry_push_secret_name = if let Some(reg_url) = node_registry_url {
        ...
        let docker_config = platform_registry::pull_secret::build_docker_config(
            reg_url,
            &agent_username,
    ...
    let valkey_creds = crate::valkey_acl::create_session_acl(
        &state.valkey,
        session_id,
    ...
    crate::pubsub_bridge::spawn_persistence_subscriber(
        state.pool.clone(),
        state.valkey.clone(),
        session_id,
    )
    ...
    if let Err(e) = pods.create(&PostParams::default(), &pod).await {
        let _ = crate::valkey_acl::delete_session_acl(&state.valkey, session_id).await;
...
pub async fn send_message(
    ...
    if session.uses_pubsub {
        crate::pubsub_bridge::publish_prompt(&state.valkey, session_id, content)
            .await
...
pub async fn stop_session(state: &AgentState, session_id: Uuid) -> Result<(), AgentError> {
    let session = fetch_session(&state.pool, session_id).await?;
    ...
    if let Some(ref ns) = session.session_namespace
        && let Err(e) = platform_k8s::namespace::delete_namespace(&state.kube, ns).await
    {
...
#[allow(dead_code)]
pub async fn run_reaper_once(state: &AgentState) {
...
pub async fn run_reaper(state: AgentState, cancel: tokio_util::sync::CancellationToken) {
...
async fn reap_terminated_sessions(state: &AgentState) -> Result<(), AgentError> {
...
/// and finalize them.
async fn reap_idle_sessions(state: &AgentState) -> Result<(), AgentError> {
    ...
    for s in idle_sessions {
        ...
        let _ = crate::valkey_acl::delete_session_acl(&state.valkey, s.id).await;
        ...
        let _ = crate::pubsub_bridge::publish_event(
            &state.valkey,
            s.id,
            &ProgressEvent {
...
pub async fn create_manager_session(
    ...
    let user_perms =
        platform_auth::resolver::effective_permissions(&state.pool, &state.valkey, user_id, None)
            .await
            .map_err(AgentError::Other)?;
...
pub async fn send_manager_message(
    ...
    tokio::spawn(async move {
        use crate::claude_cli::transport::{CliSpawnOptions, SubprocessTransport};
        ...
        let opts = CliSpawnOptions {
            ...
            system_prompt: if is_first_message {
                Some(crate::manager_prompt::MANAGER_SYSTEM_PROMPT.to_string())
            } else {
        ...
        match SubprocessTransport::spawn(opts) {
            Ok(mut transport) => {
                ...
                let result =
                    crate::cli_invoke::read_cli_output(&mut transport, session_id, &valkey).await;
            ...
            Err(e) => {
                ...
                let err_event = crate::provider::ProgressEvent {
                    kind: crate::provider::ProgressKind::Error,
                    message: format!("CLI spawn failed: {e}"),
                    metadata: None,
                };
                let _ = crate::pubsub_bridge::publish_event(&valkey, session_id, &err_event).await;
            }
        }
    });
...
async fn notify_parent_of_completion(
    ...
    if let Err(e) = crate::pubsub_bridge::publish_event(&state.valkey, parent_id, &event).await {
        tracing::warn!(
            error = %e,
            %child_session_id,
            %parent_id,
...
async fn resolve_agent_secrets(state: &AgentState, project_id: Uuid) -> Vec<(String, String)> {
    let Some(master_key_hex) = state.config.master_key.as_deref() else {
        return Vec::new();
    };
    let Ok(master_key) = platform_secrets::parse_master_key(master_key_hex) else {
        return Vec::new();
    };
    match platform_secrets::query_scoped_secrets(
        &state.pool,
        &master_key,
        project_id,
        &["agent", "all"],
...
async fn resolve_cli_oauth_token(state: &AgentState, user_id: Uuid) -> Option<String> {
    let master_key_hex = state.config.master_key.as_deref()?;
    let master_key = platform_secrets::parse_master_key(master_key_hex).ok()?;
    match platform_secrets::cli_creds::resolve_cli_auth(&state.pool, &master_key, user_id).await {
        Ok(token) => {
            if token.is_none() {
...
pub async fn resolve_user_api_key(state: &AgentState, user_id: Uuid) -> Option<String> {
    let master_key_hex = state.config.master_key.as_deref()?;
    let master_key = platform_secrets::parse_master_key(master_key_hex).ok()?;
...
/// is unavailable.
pub async fn resolve_global_api_key(state: &AgentState) -> Option<String> {
    let master_key_hex = state.config.master_key.as_deref()?;
    let master_key = platform_secrets::parse_master_key(master_key_hex).ok()?;
    match platform_secrets::resolve_global_secret(
        &state.pool,
        &master_key,
        "ANTHROPIC_API_KEY",
        "agent",
...
pub async fn resolve_active_llm_provider(
    ...
    use platform_secrets::llm_providers;
    ...
    match active.as_str() {
        ...
        v if v.starts_with("custom:") => {
            ...
            if let Some(cid) = config_id
                ...
                && let Ok(master_key) = platform_secrets::parse_master_key(master_key_hex)
            {
                match llm_providers::get_config(&state.pool, &master_key, cid, user_id).await {
                    Ok(Some(config)) if config.validation_status == "valid" => {
                        let (api_key, extra_env) = crate::llm_validate::build_provider_extra_env(
...
async fn stop_cli_session(state: &AgentState, session_id: Uuid) {
...
mod tests {
    ...
    #[test]
    fn resolve_session_namespace_uses_session_namespace() {
        let session = AgentSession {
            ...
            spawn_depth: 0,
            allowed_child_roles: None,
            execution_mode: "pod".into(),
            uses_pubsub: false,
            session_namespace: Some("project-ns-dev".into()),
        };
        assert_eq!(
            resolve_session_namespace(&session, "fallback-ns"),
            "project-ns-dev"
        );
    }

    #[test]
    fn resolve_session_namespace_uses_fallback_when_none() {
        let session = AgentSession {
            id: Uuid::new_v4(),
            project_id: None,
            user_id: Uuid::new_v4(),
        ...
        assert_eq!(
            resolve_session_namespace(&session, "platform-agents"),
            "platform-agents"
        );
    }
}
