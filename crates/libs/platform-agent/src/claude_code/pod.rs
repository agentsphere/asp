...
use crate::provider::{AgentSession, BrowserConfig, ProviderConfig};

/// Hardened security context for init containers: drop all capabilities, no privilege escalation.
...
/// Security context for the main agent container: allows privilege escalation
/// so the non-root `agent` user can use `sudo` to install packages at runtime
/// (e.g. `sudo apt-get install python3`). The security boundary is the K8s
/// namespace + `NetworkPolicy`, not Linux capabilities.
fn main_container_security() -> SecurityContext {
    SecurityContext {
...
/// No-op when `host_path` is `None`.
fn mount_host_path(host_path: Option<&str>, volumes: &mut Vec<Volume>, container: &mut Container) {
...
/// Mount the mesh proxy binary from a `hostPath` volume and wrap the container command.
/// No-op when `proxy_binary_path` is `None`.
fn mount_proxy(
    ...
    let Some(proxy_path) = proxy_binary_path else {
        return;
    };
    volumes.push(Volume {
        name: "proxy".into(),
        host_path: Some(k8s_openapi::api::core::v1::HostPathVolumeSource {
            path: proxy_path.into(),
    ...
    // Wrap original command with proxy: prepend proxy binary, then "--wrap --"
    let original_command = container.command.take().unwrap_or_default();
    let original_args = container.args.take().unwrap_or_default();
    let mut proxy_args = vec!["--wrap".to_owned(), "--".to_owned()];
    proxy_args.extend(original_command);
    proxy_args.extend(original_args);
    container.command = Some(vec!["/proxy/platform-proxy".to_owned()]);
    container.args = Some(proxy_args);
    // Add proxy env vars
    if let Some(ref mut env) = container.env {
        env.push(env_var(
            "PLATFORM_SERVICE_NAME",
            &format!("agent/{session_short_id}"),
        ));
...
pub fn build_agent_pod(params: &PodBuildParams<'_>) -> Pod {
    ...
    mount_host_path(params.host_mount_path, &mut volumes, &mut main_container);

    // Mesh proxy binary — mount from host and wrap the agent command (dev mode only)
    mount_proxy(
        params.proxy_binary_path,
        short_id,
        &mut volumes,
        &mut main_container,
    );
...
fn build_agent_runner_args(params: &PodBuildParams<'_>) -> Vec<String> {
    ...
    // Child sessions (spawned by a manager) run one-shot: process the prompt and exit.
    // Interactive sessions (user-created) stay alive for multi-turn conversation.
    if params.session.parent_session_id.is_some() {
        args.push("--one-shot".to_owned());
    }
...
fn build_env_vars(
    ...
    let mut vars = vec![
        ...
        // Explicit kubeconfig generated by setup-tools init container from the
        // mounted service account token. Ensures kubectl works reliably without
        // depending on in-cluster auto-detection.
        env_var("KUBECONFIG", "/workspace/.platform/kubeconfig"),
...
/// Build the shell script that the setup-tools init container runs.
///
/// Downloads agent-runner, installs Claude CLI, and sets up kubectl + kaniko.
/// All tools installed to `/workspace/.platform/bin/`. Idempotent.
...
fn setup_agent_runner_script() -> &'static str {
    r#"# 1. Setup agent-runner: prefer API download (always latest), fallback to baked-in
...
#[allow(clippy::uninlined_format_args)]
fn setup_claude_cli_script(claude_cli_version: &str) -> String {
    format!(
        r#"# 2. Setup Claude CLI: prefer baked-in binary, fallback to install
...
fn setup_kubectl_script() -> &'static str {
    r#"# 3. Setup kubectl: prefer baked-in binary, fallback to download
if [ ! -x "$BIN_DIR/kubectl" ] && command -v kubectl >/dev/null 2>&1; then
  ln -sf "$(command -v kubectl)" "$BIN_DIR/kubectl"
  echo "[setup] kubectl found on PATH, symlinked"
elif [ ! -x "$BIN_DIR/kubectl" ]; then
  ARCH=$(uname -m | sed 's/x86_64/amd64/;s/aarch64/arm64/')
  KUBE_VER=$(curl -sL https://dl.k8s.io/release/stable.txt 2>/dev/null || echo "v1.31.0")
  if curl -sf "https://dl.k8s.io/release/${KUBE_VER}/bin/linux/${ARCH}/kubectl" -o "$BIN_DIR/kubectl"; then
    chmod +x "$BIN_DIR/kubectl"
    echo "[setup] kubectl $KUBE_VER downloaded"
  else
    echo "[setup] WARNING: kubectl download failed — install manually if needed" >&2
  fi
fi"#
}
...
fn setup_kubeconfig_script() -> &'static str {
    r#"# 5. Generate kubeconfig from mounted service account token so kubectl works
#    without relying on in-cluster auto-detection (which Claude CLI's bash
#    sandbox may not inherit reliably).
SA_DIR=/var/run/secrets/kubernetes.io/serviceaccount
KUBECONFIG_PATH=/workspace/.platform/kubeconfig
if [ -f "$SA_DIR/token" ] && [ -f "$SA_DIR/ca.crt" ]; then
  SA_NAMESPACE=$(cat "$SA_DIR/namespace" 2>/dev/null || echo "default")
...
KCEOF
  echo "[setup] kubeconfig generated at $KUBECONFIG_PATH (namespace: $SA_NAMESPACE)"
else
  echo "[setup] WARNING: service account token not found, kubectl may not work" >&2
fi"#
}
...
fn build_setup_tools_container(
    ...
    let setup_script = build_setup_tools_script(params.claude_cli_version);
...
mod tests {
    ...
    use super::*;
    use crate::provider::{AgentSession, ProviderConfig};
    ...
    fn browser_config() -> ProviderConfig {
        ProviderConfig {
            role: Some("ui".into()),
            browser: Some(crate::provider::BrowserConfig {
    ...
    #[test]
    fn main_container_allows_privilege_escalation() {
        let session = test_session();
        let config = ProviderConfig::default();
        let pod = build_agent_pod(&PodBuildParams {
            session: &session,
        ...
        // Main container allows privilege escalation so the non-root agent user
        // can use sudo to install packages at runtime.
    ...
    #[test]
    fn build_agent_pod_with_proxy_wraps_command() {
        let session = test_session();
        let pod = build_agent_pod(&PodBuildParams {
            session: &session,
            config: &ProviderConfig::default(),
        ...
        assert_eq!(&args[2], "/workspace/.platform/bin/agent-runner");
        ...
        let svc = env
            ...
            .unwrap();
        assert_eq!(svc.value.as_deref(), Some("agent/12345678"));
        let port = env.iter().find(|e| e.name == "PROXY_HEALTH_PORT").unwrap();
        assert_eq!(port.value.as_deref(), Some("15020"));
        // Proxy volume should be present
        let volumes = spec.volumes.as_ref().unwrap();
        let proxy_vol = volumes.iter().find(|v| v.name == "proxy").unwrap();
        let hp = proxy_vol.host_path.as_ref().unwrap();
        assert_eq!(hp.path, "/host/proxy");
        // Proxy mount should be present
        let mounts = container.volume_mounts.as_ref().unwrap();
        let proxy_mount = mounts.iter().find(|m| m.name == "proxy").unwrap();
        assert_eq!(proxy_mount.mount_path, "/proxy");
        assert_eq!(proxy_mount.read_only, Some(true));
    }

    #[test]
    fn build_agent_pod_without_proxy_uses_agent_runner_directly() {
        let session = test_session();
        let pod = build_agent_pod(&PodBuildParams {
            session: &session,
            config: &ProviderConfig::default(),
        ...
        let spec = pod.spec.unwrap();
        let container = &spec.containers[0];
        // Command should be the agent-runner directly
        assert_eq!(
            container.command.as_ref().unwrap(),
            &["/workspace/.platform/bin/agent-runner"]
        );
        // No proxy env vars
        let env = container.env.as_ref().unwrap();
