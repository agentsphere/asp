...
mod tests {
    ...
    #[test]
    fn concrete_compiles_with_mock_traits() {
        // Define minimal mock impls to verify ConcretePipelineServices compiles.
        struct MockMerge;
        impl MergeRequestHandler for MockMerge {
        ...
        struct MockOps;
        impl OpsRepoManager for MockOps {
            async fn read_file(&self, _: &Path, _: &str, _: &str) -> Option<String> {
                None
            }
            async fn sync_from_project(&self, _: Uuid, _: &Path, _: &str) -> anyhow::Result<()> {
                Ok(())
            }
            async fn write_file(
                &self,
            ...
            async fn read_dir_yaml(&self, _: &Path, _: &str, _: &str) -> Option<String> {
                None
            }
        ...
        struct MockApplier;
        impl ManifestApplier for MockApplier {
            async fn render_and_apply(
                &self,
                _: &kube::Client,
                _: &str,
        ...
        struct MockRegistry;
        impl RegistryCredentialProvider for MockRegistry {
            async fn ensure_pull_secret(
                &self,
                _: &kube::Client,
                _: &str,
            ...
            async fn ensure_scoped_tokens(&self, _: Uuid, _: &str) -> anyhow::Result<()> {
                Ok(())
            }
        }

        let _svc = ConcretePipelineServices::new(
            Arc::new(MockMerge),
            Arc::new(MockWebhook),
            Arc::new(MockOps),
            Arc::new(MockApplier),
