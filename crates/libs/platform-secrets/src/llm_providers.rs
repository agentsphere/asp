...
mod tests {
    ...
    #[test]
    fn validate_env_vars_empty_key_name() {
        // Use unknown provider (no required keys) so the key validation runs
        let vars = HashMap::from([("".into(), "value".into())]);
        let result = validate_env_vars("unknown", &vars);
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("1-255 chars"));
    }

    #[test]
    fn validate_env_vars_key_too_long() {
        let long_key = "A".repeat(256);
        let vars = HashMap::from([(long_key, "value".into())]);
        let result = validate_env_vars("unknown", &vars);
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("1-255 chars"));
    }
    ...
    #[test]
    fn validate_env_vars_unknown_provider_no_required() {
        let vars = HashMap::from([("CUSTOM_KEY".into(), "value".into())]);
        assert!(validate_env_vars("unknown", &vars).is_ok());
    }

    #[test]
    fn resolved_provider_debug_no_secrets() {
        let rp = ResolvedProvider {
            oauth_token: None,
        ...
        assert!(debug.contains("extra_env_count: 0"));
    }

    #[test]
    fn provider_config_meta_serializes() {
        let meta = ProviderConfigMeta {
            id: Uuid::nil(),
            provider_type: "bedrock".into(),
            label: "test".into(),
            model: Some("claude-3".into()),
            validation_status: "untested".into(),
            last_validated_at: None,
            created_at: chrono::Utc::now(),
            updated_at: chrono::Utc::now(),
        };
        let json = serde_json::to_string(&meta).unwrap();
        assert!(json.contains("bedrock"));
