...
//!
//! Run via `just crate-test-kubernetes` (or `cargo nextest run --profile k8s`).
//! Requires a Kind cluster (`just cluster-up`).
//! File suffix `_k8s.rs` places these in the K8s tier — no `#[ignore]` needed.
...
async fn kube_client() -> kube::Client {
    // Install the ring crypto provider for rustls (kube-rs needs TLS for HTTPS)
...
async fn force_cleanup(kube: &kube::Client, ns: &str) {
    ...
    match api.delete(ns, &kube::api::DeleteParams::default()).await {
        ...
        Err(e) => eprintln!("cleanup failed for {ns}: {e}"),
    }
}
