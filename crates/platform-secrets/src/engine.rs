...
#[cfg_attr(feature = "ts", ts(export, rename = "Secret"))]
...
/// Concrete implementation of [`platform_types::SecretsResolver`] backed by Postgres.
pub struct PgSecretsResolver<'a> {
    pub pool: &'a PgPool,
    pub master_key: &'a [u8; 32],
}

impl platform_types::SecretsResolver for PgSecretsResolver<'_> {
    ...
    async fn resolve_secrets_for_env(
        ...
        resolve_secrets_for_env(self.pool, self.master_key, project_id, scope, template).await
    }
}
