...
//! Batch write functions for spans, logs, and metrics.
...
use crate::types::{LogEntryRecord, MetricRecord, SpanRecord};

/// Batch insert spans using multi-row VALUES.
///
/// Dynamic: UNNEST batch inserts — `sqlx::query!()` cannot type-check variable-length arrays.
...
pub async fn write_spans(pool: &PgPool, spans: &[SpanRecord]) -> Result<(), ObserveError> {
    ...
    // Dynamic: UNNEST batch insert — sqlx::query!() cannot type-check variable-length arrays.
    sqlx::query(
        r"
...
async fn upsert_trace(pool: &PgPool, span: &SpanRecord) -> Result<(), ObserveError> {
    ...
    if !is_root {
        ...
        sqlx::query!(
            r#"INSERT INTO traces (id, trace_id, root_span, service, status, started_at, project_id, session_id, user_id)
    ...
    sqlx::query!(
        r#"INSERT INTO traces (id, trace_id, root_span, service, status, duration_ms, started_at, finished_at,
                            project_id, session_id, user_id)
