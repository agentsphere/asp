...
mod tests {
    ...
    // -- any_value_to_json additional branches --
    ...
    fn any_value_to_json_double() {
        let val = AnyValue {
            value: Some(any_value::Value::DoubleValue(3.125)),
        };
        assert_eq!(any_value_to_json(&val), serde_json::json!(3.125));
    }
    ...
    fn any_value_to_json_kvlist() {
        ...
        assert_eq!(obj.len(), 1);
        assert_eq!(obj["k1"], serde_json::Value::Bool(false));
    }
    ...
    // -- attrs_to_json skips None value entries --

    #[test]
    fn attrs_to_json_skips_none_values() {
        let attrs = vec![
            KeyValue {
                key: "present".into(),
                value: Some(AnyValue {
        ...
        let obj = json.as_object().unwrap();
        assert_eq!(obj.len(), 1);
        assert_eq!(obj["present"], 42);
    }

    // -- get_string_attr non-string value type --

    #[test]
    fn get_string_attr_non_string_returns_none() {
        let attrs = vec![KeyValue {
            key: "num".into(),
            value: Some(AnyValue {
        ...
        assert_eq!(get_string_attr(&attrs, "num"), None);
    }

    #[test]
    fn get_string_attr_none_value_returns_none() {
        let attrs = vec![KeyValue {
            key: "empty".into(),
            value: None,
        }];
        assert_eq!(get_string_attr(&attrs, "empty"), None);
    }
