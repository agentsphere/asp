...
async fn require_workspace_admin(
    ...
    // TODO: wire from platform crate — workspace::service::is_admin
    if !workspace_is_admin(&state.pool, workspace_id, auth.user_id).await? {
        return Err(ApiError::Forbidden);
    }
    Ok(())
}
...
async fn require_workspace_member(
    ...
    if !workspace_is_member(&state.pool, workspace_id, auth.user_id).await? {
        return Err(ApiError::NotFound("workspace".into()));
    }
    Ok(())
}

// TODO: wire from platform crate — these are inline copies of workspace::service helpers
async fn workspace_is_admin(
    pool: &sqlx::PgPool,
    workspace_id: Uuid,
    user_id: Uuid,
...
// TODO: wire from platform crate — inline copy of workspace::service::is_member
async fn workspace_is_member(
    pool: &sqlx::PgPool,
    workspace_id: Uuid,
    user_id: Uuid,
) -> Result<bool, ApiError> {
...
async fn resolve_command_handler(
    ...
    validation::check_length("input", &body.input, 1, 100_000)?;
...
) -> Result<Json<Vec<platform_agent::commands::ResolvedCommandFile>>, ApiError> {
