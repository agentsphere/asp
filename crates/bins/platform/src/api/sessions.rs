...
fn validate_provider_config(config: &serde_json::Value) -> Result<(), ApiError> {
    let Ok(parsed) =
        serde_json::from_value::<platform_agent::provider::ProviderConfig>(config.clone())
    else {
        return Ok(());
    };
...
async fn update_session(
    ...
    if let Some(project_id) = body.project_id {
        ...
        let allowed = platform_auth::resolver::has_permission_scoped(
            ...
            auth.user_id,
            Some(project_id),
            platform_types::Permission::ProjectWrite,
            auth.token_scopes.as_deref(),
        )
        .await
        .map_err(ApiError::Internal)?;
...
fn session_to_response(
    session: &platform_agent::provider::AgentSession,
    truncate: bool,
) -> SessionResponse {
...
async fn sse_session_events(
    ...
    let rx = platform_agent::pubsub_bridge::subscribe_session_events(&state.valkey, session_id)
        .await
        .map_err(ApiError::Internal)?;
...
async fn replay_stored_events(
    pool: &sqlx::PgPool,
    session_id: Uuid,
) -> Vec<platform_agent::provider::ProgressEvent> {
    use sqlx::Row;
    ...
    rows.into_iter()
        .filter_map(|row| {
            ...
            let kind: platform_agent::provider::ProgressKind =
                serde_json::from_str(&kind_json).ok()?;
            Some(platform_agent::provider::ProgressEvent {
                kind,
                message: content,
                metadata,
            })
...
async fn sse_session_events_global(
    ...
    let rx = platform_agent::pubsub_bridge::subscribe_session_tree_events(&state.valkey, &all_ids)
        .await
        .map_err(ApiError::Internal)?;
...
async fn manager_session_events(
    ...
    // Subscribe to live events
    let rx = platform_agent::pubsub_bridge::subscribe_session_events(&state.valkey, id)
        .await
        .map_err(ApiError::Internal)?;
...
async fn register_pending_action(
    ...
    let event = platform_agent::provider::ProgressEvent {
        kind: platform_agent::provider::ProgressKind::SecretRequest, // Reuse SecretRequest kind for confirmation UI
    ...
    let _ = platform_agent::pubsub_bridge::publish_event(&state.valkey, id, &event).await;
...
async fn validate_manager_spawn(
    ...
    project_id: Uuid,
    parent_id: Uuid,
) -> Result<platform_agent::provider::AgentSession, ApiError> {
...
mod tests {
    ...
    fn session_to_response_no_truncation() {
        let session = platform_agent::provider::AgentSession {
            id: uuid::Uuid::new_v4(),
            project_id: None,
