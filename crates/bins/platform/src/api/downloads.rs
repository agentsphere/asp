...
async fn download_agent_runner(
    ...
    let binary_path = state.config.agent.agent_runner_dir.join(arch);
...
async fn download_mcp_servers(
    ...
    let tarball_path = &state.config.agent.mcp_servers_tarball;
...
async fn download_platform_proxy(
    ...
    let arch = normalize_arch(&params.arch)?;
    let binary_path = state.config.agent.proxy_binary_dir.join(arch);
