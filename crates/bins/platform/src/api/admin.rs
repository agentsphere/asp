...
async fn require_delegate(state: &PlatformState, auth: &AuthUser) -> Result<(), ApiError> {
...
async fn create_delegation_handler(
    ...
    // TODO: wire delegation from platform crate — delegation module not yet in platform_auth
    let params = platform_auth::resolver::CreateDelegationParams {
    ...
    let d = platform_auth::resolver::create_delegation(&state.pool, &state.valkey, &params).await?;
...
async fn revoke_delegation_handler(
    ...
    if delegator_id != auth.user_id {
        let is_admin = platform_auth::resolver::has_permission_scoped(
            &state.pool,
            &state.valkey,
            auth.user_id,
            None,
    ...
    // TODO: wire delegation from platform crate — delegation module not yet in platform_auth
    platform_auth::resolver::revoke_delegation(&state.pool, &state.valkey, id).await?;
...
async fn list_delegations(
...
) -> Result<Json<ListResponse<platform_auth::resolver::Delegation>>, ApiError> {
    require_delegate(&state, &auth).await?;
    ...
    // TODO: wire delegation from platform crate — delegation module not yet in platform_auth
    let delegations =
        platform_auth::resolver::list_delegations(&state.pool, user_id, limit, offset).await?;
