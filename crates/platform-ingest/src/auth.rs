...
//! Axum `FromRequestParts` extractor for the ingest binary.

use axum::extract::FromRequestParts;
use axum::http::request::Parts;

use platform_auth::AuthUser;
use platform_types::ApiError;

use crate::state::IngestState;

/// Newtype wrapper so we can implement `FromRequestParts<IngestState>`.
pub struct IngestAuthUser(pub AuthUser);

impl FromRequestParts<IngestState> for IngestAuthUser {
    ...
    async fn from_request_parts(
        ...
        let ip = platform_auth::extract_ip(parts, state.trust_proxy, &[]);
