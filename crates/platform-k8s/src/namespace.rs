...
//! K8s namespace lifecycle: naming, creation, RBAC, network policies, deletion.
//!
//! All operations use server-side apply (idempotent). No `AppState` dependency —
//! takes pre-extracted `kube::Client` and config values directly.
...
pub fn session_namespace_name(ns_prefix: Option<&str>, slug: &str, short_id: &str) -> String {
    match ns_prefix {
...
pub fn pipeline_namespace_name(ns_prefix: Option<&str>, slug: &str, short_id: &str) -> String {
    match ns_prefix {
...
pub fn test_namespace_name(ns_prefix: Option<&str>, slug: &str, short_id: &str) -> String {
    match ns_prefix {
...
///
/// Returns an error if the resulting slug would be empty.
...
/// Build K8s RBAC objects for an agent session namespace.
///
/// Returns `(ServiceAccount, Role, RoleBinding)` JSON objects for server-side apply.
...
/// Build a `NetworkPolicy` for a session namespace (with preview ingress on port 8000).
...
/// Build a namespace-level `NetworkPolicy` allowing egress to platform API, DNS,
/// same-namespace, mTLS to platform-managed namespaces, and public internet.
...
async fn apply_namespaced_object(
    ...
    let obj: kube::api::DynamicObject =
        serde_json::from_value(json_obj).map_err(|e| K8sError::InvalidManifest(e.to_string()))?;
...
// ---------------------------------------------------------------------------
// Ensure / delete operations
// ---------------------------------------------------------------------------

/// Ensure a session namespace exists with RBAC and `NetworkPolicy`.
///
/// Creates: Namespace + `NetworkPolicy` (unless `dev_mode`) + `ServiceAccount` +
...
/// Like `ensure_namespace` but allows specifying the services namespace explicitly.
#[allow(clippy::too_many_arguments)]
...
async fn ensure_namespace_inner(
    ...
    let obj: kube::api::DynamicObject =
        serde_json::from_value(ns_json).map_err(|e| K8sError::InvalidManifest(e.to_string()))?;
    ...
    // Create per-namespace RoleBinding for gateway access
    let gateway_rb = json!({
        "apiVersion": "rbac.authorization.k8s.io/v1",
        "kind": "RoleBinding",
        "metadata": {
...
/// Ensure the session `NetworkPolicy` exists in the given namespace.
#[tracing::instrument(skip(kube_client), fields(%ns_name), err)]
pub async fn ensure_session_network_policy(
    ...
    let obj: kube::api::DynamicObject =
        serde_json::from_value(np_json).map_err(|e| K8sError::InvalidManifest(e.to_string()))?;
...
mod tests {
    ...
    fn namespace_object_session_dev_mode_skips_psa() {
        ...
        let labels = ns["metadata"]["labels"].as_object().unwrap();
        assert!(labels.get("pod-security.kubernetes.io/enforce").is_none());
    }
    ...
    fn session_namespace_name_basic() {
        assert_eq!(
            session_namespace_name(None, "myapp", "abc12345"),
            "myapp-s-abc12345"
        );
    }
    ...
    fn session_namespace_name_with_prefix() {
        assert_eq!(
            session_namespace_name(Some("test"), "myapp", "abc12345"),
    ...
    fn session_namespace_name_long_slug() {
        ...
        let name = session_namespace_name(None, &slug, "abc12345");
        assert!(
            name.len() <= 63,
    ...
    fn session_namespace_name_always_under_63_chars() {
        ...
        let name = session_namespace_name(None, &slug, short_id);
        assert!(name.len() <= 63);
    }
    ...
    fn session_namespace_name_with_prefix_under_63_chars() {
        let slug = "a".repeat(40);
        let short_id = "12345678";
        let name = session_namespace_name(Some("pf"), &slug, short_id);
        assert!(name.len() <= 63);
    }
    ...
    fn session_network_policy_different_services_namespace() {
        ...
        let egress = np["spec"]["egress"].as_array().unwrap();
        let to = egress[0]["to"].as_array().unwrap();
        assert_eq!(to.len(), 2);
    }
    ...
    // -- pipeline/test namespace names --

    #[test]
    fn pipeline_namespace_name_basic() {
        assert_eq!(
            pipeline_namespace_name(None, "myapp", "abc12345"),
            "myapp-p-abc12345"
        );
    }

    #[test]
    fn pipeline_namespace_name_with_prefix() {
        assert_eq!(
            pipeline_namespace_name(Some("pf"), "myapp", "abc12345"),
            "pf-myapp-p-abc12345"
        );
    }

    #[test]
    fn test_namespace_name_basic() {
        assert_eq!(
            test_namespace_name(None, "myapp", "abc12345"),
            "myapp-t-abc12345"
        );
    }

    #[test]
    fn test_namespace_name_with_prefix() {
        assert_eq!(
            test_namespace_name(Some("pf"), "myapp", "abc12345"),
            "pf-myapp-t-abc12345"
        );
    }
}
