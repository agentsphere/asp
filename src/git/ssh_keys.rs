...
mod tests {
    ...
    fn test_parse_rsa_key_2048_minimum() {
        // The 4096-bit key is well above minimum, it should pass.
        // A proper 2048-bit key would also pass; we verify the threshold logic
        // with the 1024-bit rejection test below.
    ...
    fn test_parse_rsa_key_1024_rejected() {
        // Real 1024-bit RSA key (generated with ssh-keygen -t rsa -b 1024)
    ...
    fn test_parse_key_with_comment() {
        // Key has comment "test@example.com" — re-serialization produces valid OpenSSH format
        let result = parse_ssh_public_key(TEST_ED25519_KEY).unwrap();
        assert!(result.public_key_openssh.starts_with("ssh-ed25519 "));
        // The re-serialized key is a valid OpenSSH public key string
    ...
    fn test_parse_dsa_key_rejected() {
        // Real DSA key (generated with ssh-keygen -t dsa) — ssh-key 0.6 doesn't support DSA,
        // so it returns InvalidFormat at the parse stage (which is still a rejection)
