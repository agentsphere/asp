...
async fn validate_provider(
    ...
    let (api_key, extra_env) = crate::agent::llm_validate::build_provider_extra_env(
    ...
    let stream = ReceiverStream::new(rx).map(move |event| {
        let event_type = match &event {
            crate::agent::llm_validate::ValidationEvent::Test(_) => "test",
            crate::agent::llm_validate::ValidationEvent::Done { .. } => "done",
        };
